function [X, y] = converter_colunas_test(colunas_texto)  
	fprintf('Convertendo colunas para valores numericos.\n');
  
  % Carregar inputs
  if isempty(colunas_texto)
    load('preprocessamento/carregar_colunas_test.mat', 'colunas_texto', '-mat');
  end
  ...
  % Salvar outputs
  save('preprocessamento/converter_colunas_test.mat', 'X', 'y', '-mat');
end